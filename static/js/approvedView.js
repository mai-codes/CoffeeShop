document.onload = function() {
    orderLoader = new RenderApproved();
    orderLoader.load();
};
...
function RenderApproved() {
    ...
    this.updateOrderHistory = (items) => {
        $('#approvedOrdersList').empty();
        const approvedTable = $('<div class="w3-container approvedOrdersList"></div>');
        ...
        knownTotal = 0;
        $(approvedTable).append(orderDiv);
        $('#approvedOrdersList').append(orderDiv);
    }

    this.load = () => {
        $.get('/api/getApprovedOrders', {}, (data) => {
            console.log(data)
            this.update(data);
        });
    };
